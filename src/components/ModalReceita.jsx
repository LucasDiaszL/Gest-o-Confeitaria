...
export function ModalReceita({ produto, insumos, onClose, darkMode }) {
  ...
  // Cálculo de Custos com Arredondamento para evitar dízimas no lucro/custo
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-xl z-[100] flex items-center justify-center p-4">
      ...
      <div className={`relative w-full max-w-3xl rounded-[4rem] shadow-2xl border-2 overflow-hidden flex flex-col max-h-[92vh] transition-all duration-500 ${
        ...
        {/* EFEITO DE BRILHO (Apenas Dark Mode) */}
        ...
        <header className="relative z-10 p-10 pb-6 flex justify-between items-center">
          ...
          <button
            ...
            className={`p-4 rounded-3xl transition-all active:scale-90 ${
              darkMode ? "bg-white/5 text-white hover:bg-red-500/20 hover:text-red-500" : "bg-slate-100 text-slate-400 hover:bg-red-50 hover:text-red-500"
            }`}
          >
        ...
        <section className="relative z-10 px-10 py-2 grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className={`p-8 rounded-[3rem] border flex flex-col justify-center ${
          ...
          <div className={`p-8 rounded-[3rem] border flex flex-col justify-center transition-colors ${
            lucro > 0 ? "bg-emerald-500/10 border-emerald-500/20" : "bg-red-500/10 border-red-500/20"
          }`}>
            <p className={`text-[10px] uppercase font-black tracking-[0.3em] mb-2 ${lucro > 0 ? "text-emerald-500" : "text-red-500"}`}>
              {lucro > 0 ? "Margem de Lucro" : "Prejuízo Detectado"}
            </p>
            <div className="flex items-baseline gap-2">
              <span className={`text-2xl font-bold italic ${lucro > 0 ? "text-emerald-500" : "text-red-500"}`}>R$</span>
              <h3 className={`text-6xl font-black tracking-tighter ${lucro > 0 ? "text-emerald-500" : "text-red-500"}`}>
        ...
        {/* LISTA DE ITENS COM ALERTAS DE ESTOQUE */}
        <section className="relative z-10 flex-1 overflow-y-auto px-10 py-6 space-y-4">
          ...
          {carregando ? (
            <div className="text-center py-10 animate-pulse font-black text-xs text-pink-500 uppercase tracking-widest">Sincronizando Receita...</div>
          ) : (
            itens.map((item) => {
              const qtdAtual = Number(item.insumos?.quantidade_atual || 0);
              const qtdMinima = Number(item.insumos?.estoque_minimo || 5);
              const isEstoqueBaixo = qtdAtual <= qtdMinima;
                  ...
                  <div className="flex items-center gap-5">
                    ...
                    <div>
                      <h4 className={`font-black text-lg capitalize tracking-tight flex items-center gap-2 ${darkMode ? "text-white" : "text-slate-700"}`}>
                        {item.insumos?.nome}
                        {isEstoqueBaixo && (
                          <span className="text-[8px] bg-amber-500 text-white px-2 py-0.5 rounded-full uppercase font-black">Estoque Baixo</span>
                        )}
                      </h4>
                      <p className="text-xs font-bold text-slate-400 uppercase tracking-tighter">
                        Uso: {item.quantidade_utilizada}{item.insumos?.unidade_medida} — 
                        <span className={`ml-2 ${isEstoqueBaixo ? "text-amber-500" : ""}`}> 
                          Dispensa: {parseFloat(qtdAtual.toFixed(2))} {item.insumos?.unidade_medida}
                        </span>
                      </p>
                    </div>
                  </div>
                  <button
                    onClick={() => { if (confirm("Remover este ingrediente da receita?")) handleRemover(item.id); }}
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <div className="flex-1 flex gap-3">
              <select
                ...
                className={`flex-[2] p-5 rounded-3xl text-xs font-black outline-none appearance-none transition-all ${
                  darkMode ? "bg-slate-800 text-white focus:ring-2 focus:ring-pink-500" : "bg-white text-slate-600 border border-slate-200 focus:border-pink-300"
                }`}
              >
              ...
              <input
                ...
                className={`w-28 p-5 rounded-3xl text-xs font-black outline-none transition-all ${
                  darkMode ? "bg-slate-800 text-white focus:ring-2 focus:ring-pink-500" : "bg-white text-slate-600 border border-slate-200 focus:border-pink-300"
                }`}
              />
            </div>
            <button
              ...
              className="px-10 py-5 bg-pink-500 text-white rounded-3xl font-black text-xs uppercase tracking-widest hover:bg-pink-600 transition-all active:scale-95 shadow-lg shadow-pink-500/20"
            >
              Confirmar
            </button>
          </div>
