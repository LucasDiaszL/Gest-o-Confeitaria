...
export function FormNovoProduto({ onSucesso }) {
  ...
  return (
    <form
      ...
      className="bg-white p-6 rounded-4xl shadow-xl shadow-pink-100/20 border border-pink-50 grid grid-cols-1 md:grid-cols-3 gap-4 items-end animate-in zoom-in-95 duration-300"
    >
      <div className="md:col-span-1">
        <label className="block text-xs font-black text-slate-400 uppercase mb-2 ml-1 tracking-widest">
          Nome do Produto
        </label>
        <input
          type="text"
      ...
      <button
        type="submit"
        className="bg-slate-900 hover:bg-slate-800 text-white p-4 rounded-2xl font-black shadow-lg transition-all active:scale-95"
      >
        Cadastrar Produto
      </button>
    </form>
