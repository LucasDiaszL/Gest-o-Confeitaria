...
function ConteudoSistema({ user, role }) {
  ...
  // NOVO: Estado para a barra de pesquisa da Frente de Caixa
  ...
  const handleVendaManual = async (produto, metodo) => {
    try {
      ...
      // 2. VALIDAÇÃO DE FICHA TÉCNICA VAZIA 🛑 (NOVA REGRA)
      // Se não houver ingredientes cadastrados para este bolo, bloqueia!
      if (!receita || receita.length === 0) {
        mostrarToast(`Cadastre a ficha técnica de "${produto.nome}" antes de vender! 📋`, "erro");
        return; // ⛔ O "return" impede que a venda fantasma aconteça.
      }

      // 3. VALIDAÇÃO DE ESTOQUE DOS INSUMOS 🛑
      for (const item of receita) {
        // Procura o insumo atual no estado carregado do React
        const insumoNoEstoque = insumos.find((i) => i.id === item.insumo_id);
        
        // Se a quantidade atual for menor que a necessária para a receita
        if (!insumoNoEstoque || Number(insumoNoEstoque.quantidade_atual) < Number(item.quantidade_utilizada)) {
          ...
          return; // ⛔ Corta a venda se faltar ingrediente.
        }
      }
      ...
      // 6. ATUALIZA A TELA COM OS NOVOS DADOS
