...
function App() {
        ...
        for (const item of receita) {
          ...
          if (ins) {
            await supabase
              ...
              .update({
                quantidade_atual:
                  ins.quantidade_atual - item.quantidade_utilizada,
              })
              .eq("id", item.insumo_id);
          }
        }
  ...
  return (
    <div
      className={`min-h-screen flex font-sans transition-colors duration-500 ${darkMode ? "bg-slate-950 text-slate-100" : "bg-[#FDF8F9] text-slate-800"}`}
    >
      <Sidebar
        darkMode={darkMode}
      ...
      >
            ...
            <div className="space-y-10 animate-in fade-in slide-in-from-bottom-6 duration-700">
              <header className="flex justify-between items-end">
                ...
                <button
                  ...
                  className={`px-6 py-3 rounded-xl border-2 font-black text-xs transition-all ${darkMode ? "bg-slate-900 border-slate-800 text-slate-300 hover:border-pink-500" : "bg-white border-slate-100 text-slate-500 hover:border-pink-200"}`}
                >
                  🔄 Sincronizar Cloud
                </button>
              </header>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
                {produtos.map((p) => (
                  <div
                    key={p.id}
                    className={`p-8 rounded-[3.5rem] border-2 transition-all hover:-translate-y-2 ${darkMode ? "bg-slate-900 border-slate-800" : "bg-white border-slate-100 shadow-sm"}`}
                  >
                    <div className="flex flex-col items-center text-center mb-10">
                      <div
                        className={`w-24 h-24 rounded-[2.5rem] flex items-center justify-center text-5xl mb-6 shadow-inner ${darkMode ? "bg-slate-800" : "bg-pink-50/50"}`}
                      >
                        🧁
                      </div>
                      ...
                      {["Dinheiro", "PIX", "Débito", "Crédito"].map(
                        (metodo) => (
                          <button
                            ...
                            className={`py-4 rounded-3xl text-[9px] font-black uppercase tracking-widest transition-all ${darkMode ? "bg-slate-800 text-slate-400 hover:bg-pink-500" : "bg-slate-50 text-slate-500 hover:bg-pink-500"} hover:text-white active:scale-90`}
                          >
                            {metodo}
                          </button>
                        ),
            ...
            <div className="space-y-8 animate-in fade-in duration-500">
              <header className="flex justify-between items-end">
                ...
                <button
                  ...
                  className="px-8 py-4 rounded-2xl bg-slate-900 text-white font-black hover:bg-pink-500 transition-all shadow-lg"
                >
                  + Novo Insumo
                </button>
              </header>
              ...
              <Estoque
                ...
                darkMode={darkMode}
                registrarPerdaInsumo={handleRegistrarPerdaInsumo}
        ...
        {toast.show && (
          <div
            className={`fixed bottom-10 right-10 p-4 px-8 rounded-[2rem] shadow-2xl z-[150] font-black text-white animate-in slide-in-from-bottom-5 ${toast.tipo === "sucesso" ? "bg-emerald-500" : "bg-red-500"}`}
          >
            {toast.tipo === "sucesso" ? "✅ " : "❌ "}
            {toast.mensagem}
          </div>
