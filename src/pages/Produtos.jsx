...
export function Produtos({ produtos, loading, error, insumos }) {
  ...
  if (loading) return <div className="text-center py-20 animate-pulse text-pink-500 font-bold">Carregando vitrine...</div>;
  if (error) return <div className="text-red-500 text-center">Erro: {error}</div>;

  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-in fade-in duration-700">
      {produtos.map((p) => (
        <div key={p.id} className="bg-white p-6 rounded-[2rem] shadow-sm border border-pink-50 hover:shadow-md transition-all group relative">
          <button 
          ...
          <div className="bg-pink-100 w-12 h-12 rounded-2xl flex items-center justify-center mb-4 text-2xl">🧁</div>
          
          <h3 className="text-lg font-black text-slate-800 capitalize mb-1">{p.nome}</h3>
          <p className="text-slate-400 text-xs font-bold uppercase tracking-widest mb-4">Produto Final</p>
